...
export const EventDateRange = ({ event }: EventDateRangeProps) => {
    const startDateFormatted = formatDate(event.start_date, "ddd, MMM D, YYYY HH:mm", event.timezone);
    const endDateFormatted = event.end_date ? formatDate(event.end_date, "ddd, MMM D, YYYY HH:mm", event.timezone) : null;
